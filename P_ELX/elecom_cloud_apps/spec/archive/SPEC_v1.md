...
來源: EJ100_AdminLinkRequestSpecifications_APSW_v2.00Draft_20251111
更新日期: 2025-11-11
Markdown建立日期: 2026-03-11
...
| 項目 | 内容 |
|------|------|
| 要求 | ユーザーインターフェース（UI）を実装し、ユーザーによる設定変更、状態確認ができる事。 <br> Implement a user interface (UI) , so users can change settings and check the status. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクサービスに関するUIを言語設定で選択されている言語で表示する事。 <br> Displaying the UI related to AdminLink service in the language selected in the language settings. |
...
| 項目 | 内容 |
|------|------|
| 要求 | デバイスのWeb UIで、アドミリンクサービスへの登録状態を表示する事 <br> Display the registration status to the AdminLink service on the device's Web UI. |
...
| 項目 | 内容 |
|------|------|
| 要求 | デバイスのWeb UIで、アドミリンク用の設定画面を開ける事 <br> Open the AdminLink configuration screen on the device's Web UI |
...
| 項目 | 内容 |
|------|------|
| 要求 | デバイス登録状態を確認できる事。 <br> Must be able to check the device registration status. |
...
| 項目 | 内容 |
|------|------|
| 要求 | デバイスをアドミリンクサービスへ登録できる事。 <br> The device must be able to register with the AdminLink service. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクサービスからデバイスの登録を削除できる事 <br> Must be able to remove the device registration from the AdminLink service |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクサービスへデバイスを再登録できる事 <br> Must be able to re-register the device with the AdminLink service |
...
| 項目 | 内容 |
|------|------|
| 要求 | デバイス情報送信（手動によるステータス更新）ができる事 <br> Must be able to send device information (manual status update) |
...
| 項目 | 内容 |
|------|------|
| 要求 | テストイベントを送信できる事 <br> Must be able to send test events |
...
| 項目 | 内容 |
|------|------|
| 要求 | 「詳細設定画面」でアドミリンクサーバー接続用のプロキシー設定と、遠隔操作に関する設定について現在値の参照と設定変更をできる事。 <br> Must be able to refer to the current values and change the settings of the proxy settings for connecting to the AdminLink server and the remote control settings on the "Advanced Settings" screen. |
...
| 項目 | 内容 |
|------|------|
| 要求 | 設定ファイルを用いて設定を復元した場合、アドミリンクに関連する設定値を全てクリアする事。 <br> Must clear all Adminlink related values when restoring device configuration by a configuration file. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンク機能が有効且つ登録済みの間、エージェントは機能し続け、イベントの監視と定期的な処理を実行できる事。 <br> As long as the AdminLink function is enabled and registered, the agent should continue to function and it must be able to monitor events and perform periodically process. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクエージェント機能を開始できる事 <br> Must be able to start the AdminLink agent function |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクエージェント機能を停止できる事 <br> Must be able to stop the AdminLink agent function |
...
| 項目 | 内容 |
|------|------|
| 要求 | 日次処理（24時間毎）を実施できる事 <br> Must be able to carry out daily processing (every 24 hours) |
...
| 項目 | 内容 |
|------|------|
| 要求 | 1時間毎にデバイスのステータス情報を取得し、アドミリンクサーバーへ送信できる事。 <br> The device status information must be acquired hourly and sent to the AdminLink server. |
...
| 項目 | 内容 |
|------|------|
| 要求 | 1分間毎のポーリングでデバイスを監視し、イベントの発生をアドミリンクサーバーへ送信できる事。 <br> Must be able to monitor the device by polling every minute and send event occurrences to the AdminLink server. |
...
| 項目 | 内容 |
|------|------|
| 要求 | JSONデータをアドミリンクサーバーへ送信できる事 <br> Must be able to send JSON data to the Adminlink server |
...
| 項目 | 内容 |
|------|------|
| 要求 | ユーザーの指定した時間毎に接続クライアントファイルを作成し、アドミリンクサーバーへ送信できる事。 <br> A connection client file can be created and sent to the AdminLink server at the time specified by the user. |
...
| 項目 | 内容 |
|------|------|
| 要求 | Web UI の設定変更の適用を監視し、設定が変更されたらアドミリンクサーバーへ設定ファイルを送信できる事。 <br> Must be able to monitor Web UI configuration changes and send configuration file to the AdminLink server when the configuration is changed. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクサーバーからの遠隔操作要求を受信し、リクエストを実行し、結果を返信できること。 <br> Must be able to receive remote control requests from AdminLink servers, execute requests, and reply results. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンク機能が有効且つ登録済みの間、アドミリンクサーバーからの遠隔操作リクエストを受信できること。 <br> Must be able to receive remote control requests from the AdminLink server while the AdminLink function is enabled and registered. |
...
| 項目 | 内容 |
|------|------|
| 要求 | サーバーからのファイルダウンロードが必要な遠隔操作リクエストを処理できること。 <br> Functions requested via remote operation (including file downloads) shall be executable. |
...
| 項目 | 内容 |
|------|------|
| 要求 | サーバーへのファイルアップロードが必要な遠隔操作リクエストを処理できること。 <br> Functions requested via remote operation (including file upload) shall be executable. |
...
| 項目 | 内容 |
|------|------|
| 要求 | 遠隔操作で、「災害モード」の設定を変更できること。 <br> The "Emergency mode" setting must be able to be changed by remote control. |
...
| 項目 | 内容 |
|------|------|
| 要求 | アドミリンクサーバー側だけでデバイスの登録と設定をできること。 <br> Device registration and configuration shall be completable through operations on the AdminLink server side. |
...
| 項目 | 内容 |
|------|------|
| 要求 | 自動登録フロー実行判定 <br> Automatic Registration Flow Execution Judgment |
...
| 項目 | 内容 |
|------|------|
| 要求 | 自動登録フロー <br> Automatic Registration Flow |
...
| 項目 | 内容 |
|------|------|
| 要求 | ゼロタッチ待機処理 <br> Zero touch standby process |
